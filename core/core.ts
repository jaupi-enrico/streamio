...
export class Core {
   ...
   * or video resolution isn't supported for it. There is deliberately no
   * generic fallback — one used to sit here, and a family whose own branch
   * was never written would silently resolve through it, at a hardcoded
   * language, with nothing logged. A family declares `resolve` or it doesn't;
   * there is no third state to forget about.
   */
