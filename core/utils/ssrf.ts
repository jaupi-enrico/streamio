...
/**
 * The validated addresses for a URL's host, for a fetcher that cannot be
 * given a guarded socket — one that shells out to the system `curl`, say.
 * Resolving the name a second time inside that fetcher would reopen the
 * rebinding window this guard exists to close, so the caller pins the
 * connection to exactly the addresses checked here (via curl's `--resolve`,
 * or whatever equivalent it offers) instead of letting it resolve on its own.
 */
 ...
 * a per-request hook where a throw would abort everything in flight rather
